...
namespace Nemuri.UI
{
    public class OverlayPinPuzzleUI : MonoBehaviour
    {
        ...
        [Header("Image Movement Settings")]
        [SerializeField] private bool _canMoveImage1 = true;
        ...
        [Tooltip("The correct sequence of pin labels required to solve the puzzle.")]
        [SerializeField] private List<string> _correctSequence = new List<string> { "A", "B", "C" };
        ...
        private void Update()
        {
            if (_enableKeyboardToggle && Keyboard.current != null && Keyboard.current.jKey.wasPressedThisFrame)
            {
                TogglePuzzleUI();
            }
        }
        ...
        {
            if (_currentSequence.Count != _correctSequence.Count) return false;

            for (int i = 0; i < _correctSequence.Count; i++)
            {
                if (_currentSequence[i] != _correctSequence[i]) return false;
            }
            return true;
        }
        ...
        private void UpdateSequenceDisplay()
        {
            ...
            {
                _sequenceDisplayText.text = "SEQUENCE: -";
            }
            else
            {
                _sequenceDisplayText.text = "SEQUENCE: " + string.Join(" ", _currentSequence);
            }
        }
        ...
        private void EnsureUIBuilt()
        {
            ...
            _overlayContainer = new GameObject("PuzzleOverlayPanel_J", typeof(RectTransform));
